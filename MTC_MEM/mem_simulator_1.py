...
from uitility_1 import heater, compressor, flasher, spliter, HeatExchanger, mixer, mult_compressor, \
...
class Simulation:
    def __init__(
            ...
            eos,
            drop,
            aspen_app=None  # <-- 新增参数：注入全局 Aspen 对象（若需要Distiller）
    ):
        """
        :param reactors_para: 反应器参数
        :param chem_para: 化学参数（包括动力学模型等）
        :param feed_para: 进料参数
        :param mem_para: 膜组件参数
        :param eos: 状态方程 (0=ideal, 1=SRK)
        :param drop: 是否考虑压降 (1=ergun, 0=zero)
        :param aspen_app: 可选，若需要在 Simulation 内部调用 Distiller/DistillerOpt，则可传入全局 Aspen 实例
        """
        # basic info
        ...
        # ★ 关键：保存传进来的 Aspen 应用对象（若有则复用，无则为 None）
        self.aspen_app = aspen_app

        # reactor insulator feed para
        self.reactors_para = reactors_para
        self.reactor_para = self._reactor_para()
    ...
    def _feed_para(self):
        ...
        if self.feed_para["fresh"] == 1:
            ...
            if self.feed_para["H2"] == 0:
                ...
                self.F0 = x0 * self.Ft0
                self.H2 = self.F0[1] * 8.314 * 273.15 / 1e5 * 3600
            else:
                self.H2 = self.feed_para["H2"]
                self.F0[1] = self.H2 / 3600 * 1e5 / self.R / 273.15
    ...
    def _mem_para(self):
        ...
        paras_array_name = {
            'status_name': [f'status{n + 1}' for n in range(self.stage)],
        ...
        membranes = pd.DataFrame(
            index=np.arange(self.stage),
            columns=['status', 'pattern', 'pattern_h'] + paras_stage + ['nit', 'location', 'Fp', 'per']
        )

        for n in range(self.stage):
            membranes.loc[n, ['status', 'pattern', 'pattern_h'] + paras_stage] = [
                self.status[n],
                self.pattern[n],
                self.pattern_h[n],
                self.Din[n],
    ...
    def post_process_ht(self, feed, ht_stream, Tdew, permeate=None):
        ...
        sp = DistillerOpt(apk_path, block_name=block_name, feed_name=feed_name,
                          light_name=light_name, heavy_name=heavy_name)
        sp_in = feed
        sp_res_check = sp.run_RF(stream=feed, valid_comp=self.comp_list, ht_stream=ht_stream)
    ...
    def sim(self, save_profile=0, loop='direct', rtol=0.05, tol=1e-3, r_target=None):
        kn_model = self.chem_para['kn_model']
        loop = "direct" if self.recycle == 0 else loop
        if self.recycle == 1:
            res_profile, F_recycle = self.recycle_loop(
                split_ratio=0.01,
                T_cool=35 + 273.15,
                tol=tol
            )
