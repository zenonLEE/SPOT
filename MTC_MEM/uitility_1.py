...
class DistillerOpt:
    def __init__(self, apk_name, block_name, feed_name, light_name, heavy_name, aspen_app=None):
        """
        :param apk_name: Aspen bkp 文件路径
        :param block_name: Aspen Distillation block 名
        :param feed_name: 进料流名
        :param light_name: 轻组分流名
        :param heavy_name: 重组分流名
        :param aspen_app: 若已在外部初始化 Aspen，可传此对象进来；否则本类会自己打开/关闭 Aspen。
        """
        ...
        self.aspen_app = aspen_app  # ★ 如果不为空，就用它；为空则内部自己打开

        self.apk_comp = []
        # 如果没有 aspen_app，则自行打开 Aspen；否则复用外部 Aspen
        if self.aspen_app is None:
            self._init_apk(apk_name)
        else:
            # 已有 Aspen 实例，跳过 _init_apk 内的 EnsureDispatch 和 InitFromArchive2
            # 但仍需从该 aspen_app 里获取 "Tree" 等属性
            self.aspen = self.aspen_app
            # 假设外部脚本已经 InitFromArchive2(...) 了, 并有相应的 BKP 文件加载
            # 如果还没加载, 也可以在这里 self.aspen.InitFromArchive2(apk_name)
            # 但一般由外部决定
    ...
    def _init_apk(self, apk):
        """
        如果没有外部 aspen_app，则 DistillerOpt 自己打开 Aspen 并加载 bkp
        """
        try:
            ...
            print(f"An error occurred in _init_apk: {e}")
            # 打开失败则退出
    ...
    def _get_comp(self):
        ...
        try:
            ...
            if self.aspen_app is None and self.aspen is not None:
                self.aspen.Quit()  # 只在本类自己打开时才 Quit
            exit(1)

    def set_feed_parameters(self, stream_name=None, stream_source=None):
        """
        设置 Aspen 中进料流的温度、压力、组分
        """
    ...
    def set_block_parameters(self, block_para=None, block_name=None):
        """
        给 distillation block 设置相关参数（如 NSTAGE, RR等）
        如果 block_para 不给，就先做 opt_distiller()
        """
    ...
    def run_apk(self, stream=None, valid_comp=None, quit_aspen=True):
        ...
        - access_to_rf
        - access_to_st
        - 如果 quit_aspen=True 并且我们本类自己打开了 Aspen, 则 self.aspen.Quit()
        """
        ...
        # 如果 aspen_app is None，意味着本类自己开 Aspen; => 需要自己关
        # 如果 aspen_app is not None，就不要 Quit, 由外部管理
        if quit_aspen and (self.aspen_app is None):
            self.aspen.Quit()
    ...
    def run_RF(self, stream=None, valid_comp=None, opt=True, quit_aspen=True, ht_stream=None):
        ...
        更常用的入口:
        - set_feed_parameters
        - (opt) -> self.opt_distiller()
        - self.set_block_parameters
        - 如果 ht_stream:
            self.set_feed_parameters(stream_name='S11', stream_source=ht_stream)
        - self.aspen.Engine.Run2()
        - ...
        - (根据 quit_aspen 和 aspen_app 决定是否 Quit)
        """
        valid_comp = self.apk_comp if valid_comp is None else valid_comp
        ...
        # 6) 若本类自己开的 Aspen, 并且 quit_aspen=True -> Quit
        if quit_aspen and (self.aspen_app is None):
            self.aspen.Quit()
...
class Distiller:
    ...
    def opt_distiller(self):
        opt_block ="E:\project\lsj_y_CH3OH_C\MTC_MEM\DT_with_MR_opt.bkp"
        block_name = 'B1'
        block_node = self.aspen.Tree.FindNode(f"\\Data\\Blocks\\{block_name}\\Input")
        block_node.Elements("OPT_NTRR").Value = 'NSTAGE'
        # nstages = np.arange(9)
